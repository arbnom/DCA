...
def dca_sell_strategy(file_path, start_date, end_date, period, selling_amount, selling_duration, first_sell_periods_after_buy):
    ...
    # Debug: Print filtered data range
    ...
    while initial_buy_date + timedelta(days=(first_sell_periods_after_buy + selling_duration - 1) * period) <= end_date_dt:
        initial_price_data = filtered_data.loc[filtered_data['Date'] == initial_buy_date, 'Price']
        
        if initial_price_data.empty:
            print(f"No price data found for initial buy date: {initial_buy_date.strftime('%Y-%m-%d')}")
            initial_buy_date += timedelta(days=1)
            continue
        
        initial_price = initial_price_data.values[0]
        ...
        while current_date <= last_sold_date:
            ...
            if not price_on_date.empty:
                ...
                print(f"No price data found for sell date: {current_date.strftime('%Y-%m-%d')}")
        ...
        # Calculate hold value and ROI
        end_price_data = filtered_data.loc[filtered_data['Date'] == last_sold_date, 'Price']
        if not end_price_data.empty:
            end_price = end_price_data.values[0]
            hold_value = total_eth_to_sell * end_price
            roi_hold = ((hold_value - initial_buying_amount) / initial_buying_amount) * 100
            ...
            results.append({
                ...
                'ROI': roi,
                'Hold Value at End': hold_value,
                'ROI Hold': roi_hold
            })
        else:
            print(f"No price data found for last sell date: {last_sold_date.strftime('%Y-%m-%d')}")
